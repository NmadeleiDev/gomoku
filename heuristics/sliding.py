...
def get_board_sliding_indices(line_len_to_analyze=5):
    ...
    straight_line_indices_0 = [np.arange(line_len_to_analyze) + i for i in range(window_path_len)] * Board.size
    straight_line_indices_1 = np.concatenate([np.ones((window_path_len,)) * i for i in range(Board.size)]).astype(
        int)[:, np.newaxis]

    diags = [np.arange(line_len_to_analyze) + i for i in range(window_path_len)]
    inv_diags = [np.flip(np.arange(line_len_to_analyze)) + i for i in range(window_path_len)]

    diag_indices_0, diag_indices_1 = [list(x) for x in zip(*list(product(diags, diags + inv_diags)))]

    return straight_line_indices_0, straight_line_indices_1, diag_indices_0, diag_indices_1
...
def score_for_line_hamming(color: int, line: tuple) -> float:
    ...
    for color_in_line in np.unique(line):
        ...
        ham_sim = hamming_similarity(line, win_line_for_color)  # TODO различие с пустым местом и камнем оппонента учитывать по-разному
...
def score_for_line_count_with_move(color: int, line: tuple, whos_move: int) -> float:
    ...
    if len(unique_colors) == 3 or (len(unique_colors) == 2 and Board.empty_color not in unique_colors):
        return 0
    ...
    score = None
    if (is_this_color_line and color == whos_move) or (not is_this_color_line and color != whos_move):
        if count == 3 and line[0] == Board.empty_color and line[-1] == Board.empty_color:
            score = np.inf
        elif count == 4 and (line[0] == Board.empty_color or line[-1] == Board.empty_color):
            score = np.inf
        elif count == 5:
            score = np.inf
    else:
...
def apply_heuristic(h_fn, whos_move: int, board: Board):
    straight_line_indices_0, straight_line_indices_1, diag_indices_0, diag_indices_1 = get_board_sliding_indices()

    position = board.position

    lines = np.concatenate([position[straight_line_indices_0, straight_line_indices_1],
    ...
    result = sum([h_fn(tuple(x), whos_move) * count for x, count in zip(unique_lines, unique_counts)])
    return result


def build_heuristic(color: int, scorer_type: Literal['hamming', 'count', 'count_with_move', 'bin'] = "count"
